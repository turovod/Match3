...
public class SM_Play : MonoBehaviour
{
    ...
    void InitField()
    {
        ...
        for (int y = 0; y < 7; y++)
        {
            for (int x = 0; x < 7; x++)
            {
                GameObject _g = Instantiate(oFigure);
                _g.SetActive(false);
                int _index = GetIndex(x,y);
                Figure _figure = _g.GetComponent<Figure>();
                _figure.Init(_index);
                field[x, y] = _figure;
    ...
    int GetIndex(int x_,int y_)
    {
        ...
        else
        {
            if (x_ == 0)
            {
                while (_index == field[x_, y_-1].index || _index == field[1, 0].index ) _index = Random.Range(0, txFigure.Length);
            }
            else if (x_ == 6)
            {
                while (_index == field[x_, y_ - 1].index || _index == field[5, 0].index) _index = Random.Range(0, txFigure.Length);
            }
            else
            {
                while (_index == field[x_, y_ - 1].index || _index == field[x_ - 1, 0].index || _index == field[x_ + 1, 0].index) _index = Random.Range(0, txFigure.Length);
            }
        }
        return _index;
    }
